...
const config = {
  ...
  vendor: [
    ...
    'app/vendor/selectFilter/selectFilter.js',
    'app/vendor/layer/layer.js',
    'app/vendor/art-template.js',
    'app/vendor/jquery.nav.js',
    'app/vendor/city-picker.js',
  ],
  vendor_css: [
    ...
    colors.forEach(e => {
      gulp.src('app/styles/*.scss')
        ...
        .pipe($.sass.sync({
          outputStyle: 'compressed', //'expanded'
          precision: 10,
          includePaths: ['./app/styles/template/' + e]
        }).on('error', $.sass.logError))
        .pipe($.autoprefixer({browsers: ['> 1%', 'last 2 versions', 'Firefox ESR']}))
        .pipe($.if(dev, $.sourcemaps.write()))
        .pipe($.if(dev, gulp.dest('.tmp/styles/'+ e), gulp.dest('dist/styles/'+e)))
        .pipe(reload({stream: true}));
    })
});

gulp.task('scripts', () => {
  gulp.src(['app/vendor/layer/**/*']).pipe($.if(dev, gulp.dest('.tmp/scripts/layer'), gulp.dest('dist/scripts/layer')));
...
gulp.task('vendorJs', () => {
  gulp.src('app/vendor/**/*').pipe($.if(dev,gulp.dest('.tmp/vendor'),gulp.dest('dist/vendor')));
  gulp.src('app/vendor/jquery/jquery.min.js').pipe($.if(dev,gulp.dest('.tmp/scripts'),gulp.dest('dist/scripts')));
...
//整体打包
gulp.task('build-start', ['vendorJs','html', 'images', 'styles', 'fonts', 'extras'], () => {
